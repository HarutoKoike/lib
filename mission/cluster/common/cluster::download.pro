...
;
; root directory
root_dir = GETENV('CLUSTER_DATA_PATH')
IF STRLEN(root_dir) EQ 0 THEN CD, CURRENT=root_dir
; subdirectory
separator = PATH_SEP()
;
count = 0
...
;
odir = STRSPLIT(untar_files[0], separator, /EXTRACT)
odir = FILE_DIRNAME(untar_files[0])
odir = STRSPLIT(odir, separator, /EXTRACT)
odir = separator + STRJOIN(odir[0:-2], separator) 
